...
import { resolveFragmentShader } from "@/lib/shaders/post"
...
describe("shared GLSL chunks", () => {
  it("puts one definition of each conversion in both shaders", () => {
    for (const chunk of [colorSpaceChunk, ditherChunk]) {
      expect(organicGradientFragmentShader).toContain(chunk.trim())
      expect(resolveFragmentShader).toContain(chunk.trim())
    }
  ...
  it("defines each shared function exactly once per shader", () => {
    ...
    for (const shader of [organicGradientFragmentShader, resolveFragmentShader]) {
